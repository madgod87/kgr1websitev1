...
export default function PhotoManagement() {
  ...
  const handleDeleteSelected = async () => {
    ...
    const confirmed = confirm(`Are you sure you want to delete ${selectedImages.size} selected image(s)?`)
    if (!confirmed) return
    ...
    try {
      ...
      setSuccess(`Successfully deleted ${selectedImages.size} image(s)`)
      setSelectedImages(new Set())
      loadImages() // Refresh the list
    } catch (error) {
                      ...
                      <div className="relative aspect-square">
                        ...
                        <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-200 rounded-lg flex items-end">
                          <div className="p-3 text-white text-xs opacity-0 group-hover:opacity-100 transition-opacity">
                            <p className="font-medium truncate">{image.alt_text || 'Untitled'}</p>
